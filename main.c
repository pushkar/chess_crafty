 ...
 *   fails to recognize a command, then try InputMove() to   *
    ...
    do {
      ...
      do {
        ...
        if (presult == 0 || presult == 2) {
        	//TODO
          ACHRecordBoardStateNext((char*)display.board);
          ACHWrite();
          ACHRead();
          if (!xboard) {
 ...
 */
      ...
      if (xboard)
        printf("move %s\n", OutputMove(tree, last_pv.path[1], 0, wtm));
      // TODO
      ACHRecordMove(OutputMoveExtended(tree, last_pv.path[1], 0, wtm));
      ACHRecordBoardStatePrev((char*)display.board);
      PipeWrite(OutputMoveExtended(tree, last_pv.path[1], 0, wtm));
