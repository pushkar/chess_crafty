...
void PipeOpen() {
  if(pipeo == 0) {
    Print(128, "PIPES: Waiting for client to start...\n");
    input_pipe = open(INPUT_PIPE, O_RDONLY);
    output_pipe = open(OUTPUT_PIPE, O_WRONLY);
    Print(128, "PIPES: Done\n");
  }
  else
	Print(128, "PIPES: Pipe is already open\n");
}

void PipeWrite(char* buf) {
	if(pipeo)
		write(output_pipe, buf, strlen(buf));
}

int PipeRead(char* buf) {
	int ret = 0;
	if(pipeo) {
		memset(buf, 0, 1024);
		ret = read(input_pipe, buf, 1024);
	}
	return ret;
}

void PipeClose() {
	Print(128, "PIPES: Closing pipes\n");
	close(input_pipe);
	close(output_pipe);
}
...
int ReadInput(void) {
  ...
  // TODO
  bytes = PipeRead(buffer);
  // bytes = ACHRead_In(buffer);
  if(bytes <= 0) {
    do
